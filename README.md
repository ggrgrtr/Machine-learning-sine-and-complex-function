...
**->** Функция активации (пороговая, сигмоидная, Tanh(x), ReLU, ELU, SoftMax - для вероятностей многоклассовой классификации), определяющая результат работы нейрона по весам и аргументу
...
**->** Определение скрытых нейронных слоев (полносвязные) и выходного слоя
...
**->** Свертка (Сверточный слой, pooling слой)

**Борьба с переобучением:**

**->** Dropout (drop connection, drop neuron)

**->** Регуляризация -- ограничение свободы сети в выборе весов, штраф за большие коэффициенты. (LASSO, Тихонова)

**->** Аугментация данных (раширение, преобразование и т.д.)

**->** Early stopping -- определение и приостановка эпохи, с которой начинается переобучение
...
Свертка позволяет обобщить данные (срез лишней информации) и не зависеть от положения нужного объекта на картинке
...
**Точки - вина определенного класса; области - определение/предсказание н.сетью класса вина**
